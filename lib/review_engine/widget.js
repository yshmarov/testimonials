...
(function () {
  ...
  var state = null; // one open session: { kind, auto, rating, videoBlob, ... }
  var media = { stream: null, recorder: null, chunks: [], timer: null, url: null };
  ...
  function renderInline() {
    ...
    card.appendChild(formStage(config.labels.shareTitle));
  }
  ...
  function formStage(title) {
    ...
    if (!config.authenticated) appendGuestFields(form);
  ...
  function videoControl(form) {
    ...
    if (state.videoBlob) {
      ...
      chip.appendChild(el("span", null, "🎥 " + config.labels.videoAttached));
      var remove = el("button", "rve-plain", config.labels.remove);
      remove.type = "button";
    ...
    var button = el("button", "rve-secondary rve-record", "🎥 " + config.labels.recordVideo);
    button.type = "button";
    button.addEventListener("click", function () {
  ...
  // Recording lives in its own stage: camera check -> recording -> review.
  function videoStage() {
    var stage = el("div", "rve-stage");
    ...
    stage.appendChild(el("p", "rve-hint", config.labels.videoHint));
  ...
  function stopMedia() {
    ...
    media = { stream: null, recorder: null, chunks: [], timer: null, url: null };
  }
  ...
  function injectStyles() {
    ...
    var css = [
      ...
      ".rve-record{display:inline-flex;align-items:center;gap:6px}",
      ".rve-chip{display:flex;align-items:center;justify-content:space-between;gap:8px;",
      "padding:8px 12px;border:1px dashed #d1d5db;border-radius:8px}",
      ".rve-video{width:100%;border-radius:10px;background:#000;aspect-ratio:4/3}",
      ".rve-timer{text-align:center;color:#dc2626;font-weight:600;margin:6px 0}",
      ...
      ".rve-chip{border-color:#2a313a}",
      ".rve-hint,.rve-nps-legend{color:#9aa2ab}",
      "}"
    ].join("");
