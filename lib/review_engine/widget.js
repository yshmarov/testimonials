/*
 * review_engine widget — self-contained, no framework, no build step.
 *
 * Reads its config from the <script type="application/json"
 * data-review-engine-config> the server renders — re-read on every render so
 ...
 * `review_prompt!` arrives as config.autoOpen), from any element carrying
 * `data-review-prompt` (or `data-review-prompt="nps"`), or from
 * window.ReviewEngine.open() / .openNps().
 *
 ...
 * <div data-review-engine-inline> (config.mode === "page").
 */
(function () {
  "use strict";

  var config = readConfig();
  if (!config || window.__reviewEngineLoaded) return;
  window.__reviewEngineLoaded = true;
  ...
  function readConfig() {
    var el = document.querySelector("script[data-review-engine-config]");
    if (!el) return null;
    try {
  ...
  function render() {
    ...
    var el = document.querySelector("script[data-review-engine-config]");
    if (config.autoOpen && el && !el.dataset.reviewEngineHandled) {
      el.dataset.reviewEngineHandled = "1";
      setTimeout(function () {
  ...
  function handleOpenerClick(event) {
    var opener = event.target && event.target.closest
      ? event.target.closest("[data-review-prompt]")
      : null;
    if (!opener) return;
    event.preventDefault();
    event.stopPropagation();
    if (opener.getAttribute("data-review-prompt") === "nps") openNps(false);
  ...
  function dialogEl() {
    return overlay ? overlay.querySelector("#rve-dialog") : document.querySelector("[data-review-engine-inline] .rve-inline");
  }
  ...
  function renderInline() {
    var host = document.querySelector("[data-review-engine-inline]");
    if (!host || host.querySelector(".rve-inline")) return;
    var card = document.createElement("div");
    card.className = "rve-inline";
  ...
  function rateStage() {
    state.stage = "rate";
    var stage = el("div", "rve-stage");
    stage.appendChild(el("h2", "rve-title", config.labels.enjoying));

    var stars = el("div", "rve-stars rve-stars-big");
    for (var i = 1; i <= 5; i++) {
  ...
  function paintStars(container, upto) {
    var stars = container.querySelectorAll(".rve-star");
    for (var i = 0; i < stars.length; i++) {
      stars[i].classList.toggle("rve-star-on", i < upto);
    }
  }

  function notNowButton() {
    var button = el("button", "rve-plain", config.labels.notNow);
    button.type = "button";
  ...
  function formStage(title) {
    ...
    var error = el("p", "rve-error");
    error.hidden = true;
    form.appendChild(error);

    var actions = el("div", "rve-actions");
    actions.appendChild(secondaryButton(config.labels.cancel, close));
    var submit = el("button", "rve-primary", config.labels.submit);
    submit.type = "submit";
  ...
  function videoControl(form) {
    var wrap = el("div", "rve-field");

    if (state.videoBlob) {
      var chip = el("div", "rve-chip");
      chip.appendChild(el("span", "rve-chip-label", "🎥 " + config.labels.videoAttached));
      var remove = el("button", "rve-chip-remove", config.labels.remove);
      remove.type = "button";
    ...
    var button = el("button", "rve-record");
    button.type = "button";
    button.appendChild(el("span", "rve-record-label", "🎥 " + config.labels.recordVideo));
    button.appendChild(el("span", "rve-record-hint", config.labels.recordVideoHint));
    button.addEventListener("click", function () {
  ...
  function videoStage() {
    ...
    var frame = el("div", "rve-preview");
    var preview = document.createElement("video");
    preview.className = "rve-video";
    preview.autoplay = true;
    preview.muted = true;
    preview.playsInline = true;
    frame.appendChild(preview);

    var timer = el("div", "rve-timer");
    timer.hidden = true;
    frame.appendChild(timer);

    var countdown = el("div", "rve-countdown");
    countdown.hidden = true;
    frame.appendChild(countdown);
    stage.appendChild(frame);

    var hint = el("p", "rve-hint", config.labels.videoHint);
    ...
    var actions = el("div", "rve-actions");
    var start = el("button", "rve-primary", config.labels.startRecording);
    start.type = "button";
    var stop = el("button", "rve-primary", config.labels.stopRecording);
    stop.type = "button";
    stop.hidden = true;
    ...
    var upload = el("button", "rve-plain", config.labels.uploadInstead);
    upload.type = "button";
  ...
  function reviewStage(blob) {
    ...
    var use = el("button", "rve-primary", config.labels.useVideo);
    use.type = "button";
  ...
  function npsButton(score) {
    var button = el("button", "rve-nps-score", String(score));
    button.type = "button";
    button.addEventListener("click", function () {
      state.rating = score;
  ...
  function npsCommentStage(score) {
    ...
    var picked = el("div", "rve-nps-picked", String(score));
  ...
  function submitTestimonial(form) {
    ...
    if (!body && !state.videoBlob) return showError(form, config.labels.errorBlank);
  ...
  function postForm(endpoint, data, form, onSuccess) {
    var submit = form.querySelector(".rve-primary");
  ...
  function thanks() {
    ...
    if (!dialog) return;
    dialog.textContent = "";
    var note = el("div", "rve-thanks");
    note.appendChild(el("div", "rve-thanks-mark", "✓"));
  ...
  function header(title) {
    var head = el("div", "rve-head");
    head.appendChild(el("h2", "rve-title", title));
    if (overlay) {
      var closeButton = el("button", "rve-x", "×");
  ...
  function secondaryButton(text, onClick) {
    var button = el("button", "rve-secondary", text);
    button.type = "button";
  ...
  function showError(form, text) {
    var error = form.querySelector(".rve-error");
    if (!error) return;
  ...
  function injectStyles() {
    ...
    var css = [
      "#rve-overlay{position:fixed;inset:0;z-index:" + Z + ";background:rgba(0,0,0,.45);",
      "display:flex;align-items:center;justify-content:center;padding:16px}",
      "#rve-dialog,.rve-inline{width:100%;max-width:440px;max-height:92vh;overflow:auto;",
      ...
      ".rve-inline{box-shadow:none;border:1px solid #e5e7eb;max-width:520px;margin:0 auto}",
      ".rve-head{display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin:0 0 12px}",
      ".rve-title{margin:0;font-size:17px}",
      ".rve-x{border:0;background:none;font-size:22px;line-height:1;cursor:pointer;color:inherit;padding:2px 6px}",
      ".rve-stage>.rve-title{display:block;text-align:center;margin-bottom:12px}",
      ".rve-stars{display:flex;justify-content:center;gap:4px;margin:4px 0 12px}",
      ".rve-star{border:0;background:none;cursor:pointer;font-size:26px;line-height:1;padding:2px;color:#d1d5db}",
      ".rve-stars-big .rve-star{font-size:38px}",
      ".rve-star-on{color:#f59e0b}",
      ".rve-plain{display:block;margin:4px auto 0;border:0;background:none;cursor:pointer;",
      "color:#2563eb;font:inherit;padding:6px}",
      ".rve-questions{margin:0 0 12px;padding:10px 12px;border-radius:10px;background:rgba(37,99,235,.07);font-size:13px}",
      ".rve-questions strong{display:block;margin-bottom:4px;font-size:11px;text-transform:uppercase;letter-spacing:.05em;color:#2563eb}",
      ".rve-questions ul{margin:0;padding-inline-start:18px}",
      ".rve-field{display:block;margin-bottom:12px}",
      ".rve-field>span{display:block;margin-bottom:4px;font-weight:600}",
      "#rve-dialog textarea,#rve-dialog input[type=text],#rve-dialog input[type=email],#rve-dialog input[type=file],",
      ".rve-inline textarea,.rve-inline input[type=text],.rve-inline input[type=email],.rve-inline input[type=file]",
      ...
      ".rve-stage textarea{resize:vertical}",
      ".rve-consent{display:flex;gap:8px;align-items:flex-start;margin-bottom:12px;font-size:13px;cursor:pointer}",
      ".rve-consent input{margin-top:3px}",
      ".rve-error{color:#dc2626;margin:0 0 12px}",
      ".rve-hint{color:#6b7280;font-size:13px;margin:8px 0}",
      ".rve-actions{display:flex;justify-content:flex-end;gap:8px;flex-wrap:wrap}",
      ".rve-actions button{padding:8px 14px;border-radius:8px;cursor:pointer;font:inherit}",
      ".rve-actions .rve-plain{margin:0}",
      ".rve-secondary{border:1px solid #d1d5db;background:none;color:inherit;padding:8px 14px;border-radius:8px;cursor:pointer;font:inherit}",
      ".rve-primary{border:0;background:#2563eb;color:#fff;font-weight:600;padding:8px 14px;border-radius:8px;cursor:pointer;font:inherit}",
      ".rve-primary:disabled{opacity:.6;cursor:default}",
      ".rve-record{display:flex;flex-direction:column;align-items:center;gap:2px;width:100%;",
      "padding:12px;border:1.5px dashed #2563eb;border-radius:10px;cursor:pointer;",
      "background:rgba(37,99,235,.06);font:inherit}",
      ".rve-record:hover{background:rgba(37,99,235,.12)}",
      ".rve-record-label{color:#2563eb;font-weight:600}",
      ".rve-record-hint{font-size:12px;color:#6b7280}",
      ".rve-chip{display:flex;align-items:center;justify-content:space-between;gap:8px;",
      "padding:10px 12px;border-radius:10px;background:rgba(37,99,235,.07)}",
      ".rve-chip-label{font-weight:600}",
      ".rve-chip-remove{border:0;background:none;color:#2563eb;cursor:pointer;font:inherit;padding:2px 6px}",
      ".rve-video{display:block;width:100%;border-radius:10px;background:#000;aspect-ratio:4/3;margin:0 0 12px}",
      ".rve-preview{position:relative;margin-bottom:4px}",
      ".rve-preview .rve-video{margin-bottom:0}",
      ".rve-timer{position:absolute;top:8px;inset-inline-end:8px;background:rgba(0,0,0,.55);color:#fff;",
      "padding:2px 10px;border-radius:999px;font-weight:600;font-size:13px}",
      ".rve-countdown{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;",
      "font-size:72px;font-weight:700;color:#fff;text-shadow:0 2px 16px rgba(0,0,0,.6)}",
      ".rve-nps{display:flex;gap:4px;justify-content:center;flex-wrap:wrap;margin:4px 0 6px}",
      ".rve-nps-score{min-width:32px;padding:8px 0;border:1px solid #d1d5db;border-radius:8px;",
      "background:none;color:inherit;cursor:pointer;font:inherit}",
      ".rve-nps-score:hover{background:#2563eb;border-color:#2563eb;color:#fff}",
      ".rve-nps-legend{display:flex;justify-content:space-between;font-size:12px;color:#6b7280;margin-bottom:8px}",
      ".rve-nps-picked{width:44px;height:44px;margin:0 auto 12px;display:flex;align-items:center;justify-content:center;",
      "border-radius:10px;background:#2563eb;color:#fff;font-size:18px;font-weight:700}",
      ".rve-thanks{text-align:center;padding:16px 0}",
      ".rve-thanks-mark{width:44px;height:44px;margin:0 auto 10px;display:flex;align-items:center;justify-content:center;",
      "border-radius:50%;background:#16a34a;color:#fff;font-size:22px}",
      "@media (prefers-color-scheme:dark){",
      "#rve-dialog,.rve-inline{background:#1a1f26;color:#e6e8ea}",
      ".rve-inline{border-color:#2a313a}",
      "#rve-dialog textarea,#rve-dialog input[type=text],#rve-dialog input[type=email],#rve-dialog input[type=file],",
      ".rve-inline textarea,.rve-inline input[type=text],.rve-inline input[type=email],.rve-inline input[type=file]",
      "{border-color:#2a313a}",
      ".rve-secondary{border-color:#2a313a}",
      ".rve-star{color:#3a4149}",
      ".rve-star-on{color:#f59e0b}",
      ".rve-nps-score{border-color:#2a313a}",
      ".rve-record{border-color:#3b82f6;background:rgba(59,130,246,.1)}",
      ".rve-record:hover{background:rgba(59,130,246,.18)}",
      ".rve-record-label,.rve-chip-remove{color:#3b82f6}",
      ".rve-chip{background:rgba(59,130,246,.12)}",
      ".rve-hint,.rve-nps-legend,.rve-record-hint{color:#9aa2ab}",
      "}"
    ].join("");
