...
RSpec.describe ReviewEngine::PromptEvent do
  def eligible?(**args)
  ...
  it 're-allows NPS after nps_reprompt_after' do
    ...
    described_class.update_all(created_at: Time.current - ReviewEngine.config.nps_reprompt_after - 60)
  ...
  it 'blocks after a recent dismissal, then relents' do
    ...
    described_class.update_all(created_at: Time.current - ReviewEngine.config.reprompt_after - 60)
    expect(eligible?(author_id: '42')).to be(true)
  end

  it 'stops for good after max_prompts shows' do
    ReviewEngine.config.max_prompts = 2
